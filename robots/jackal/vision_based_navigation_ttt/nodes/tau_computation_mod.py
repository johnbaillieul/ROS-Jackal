...
from vision_based_navigation_ttt.msg import OpticalFlow, TauComputation
...
def draw_image_segmentation(curr_image, tau_el, tau_er, tau_l, tau_r, tau_c, error_c, error_el, error_er, error_l, error_r):
    ...
    cv2.putText(color_image, str(round(error_l, 1)),
                (int((x_end_l + x_init_l) / 2.1), int((y_end_l + y_init_l) / 2)-50),
                font, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    cv2.putText(color_image, str(round(error_r, 1)),
                (int((x_end_r + x_init_r) / 2.1), int((y_end_r + y_init_r) / 2)-50),
                font, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    cv2.putText(color_image, str(round(error_c, 1)),
                (int((x_end_c + x_init_c) / 2.1), int((y_end_c + y_init_c) / 2)-50),
                font, 1, (0, 0, 255), 2, cv2.LINE_AA)
...
class TauComputationClass:
    ...
    def callback_of(self, data):
        ...
        error_el = np.std(tau_left_e)**2
        final_tau_right_e = tau_final_value(self, tau_right_e, count_right_e)
        error_er = np.std(tau_right_e)**2
        print("Tau right Extreme: " + str(tau_right_e))
        print("Tau left Extreme: " + str(tau_left_e))
        # Right and left
        final_tau_left = tau_final_value(self, tau_left, count_left)
        error_l = np.std(tau_left)**2
        final_tau_right = tau_final_value(self, tau_right, count_right)
        error_r = np.std(tau_right)**2
        ...
        draw_image_segmentation(self.curr_image, final_tau_left_e, final_tau_right_e, final_tau_left, final_tau_right, final_tau_centre,error_c,error_el,error_er,error_l,error_r)
