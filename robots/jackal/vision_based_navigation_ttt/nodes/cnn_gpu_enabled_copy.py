...
class train_5_without_v():
    ...
    def train_(self):
        ...
        for folder in folders:
            ...
            for idx in range(1) :#len(images_in_folder) - 
                ...
                try:
                    ...
                    # image_as_array_1 = image_as_array_1.reshape(image_size ,image_size ,3)
        ...
        input_1 = keras.layers.Input(shape=(image_size ,image_size ,6))
        ...
        epochs = 100
        ## train the model
        model.fit({"input_1": X_train}, y_train, batch_size = batch_size, epochs = epochs,  validation_data=({"input_1": X_valid}, y_valid))
...
class train_5_with_v():
    ...
    def train_(self):
        ...
        print(model.summary())
        # # plot graph
        model_name = 'model_5_with_v_flag_img_size_' + str(image_size) 
        keras.utils.plot_model(model, model_name + '.png', show_shapes=True)
        earlystopping = keras.callbacks.EarlyStopping(monitor ="val_loss", 
                                        mode ="min", patience = 5, 
                                        restore_best_weights = True)
        ...
        epochs = 100
        ## train the model
        model.fit({"input_1": X_train, "input_2": v_train}, y_train, batch_size = batch_size, epochs = epochs,  validation_data=({"input_1": X_valid, "input_2": v_valid}, y_valid), callbacks =[earlystopping])
        model.save("model_keras.h5")
        
        # print(model.predict(X_test)[8])
        print(model.evaluate({"input_1": X_test, "input_2": v_test}, y_test))
        print(model.metrics_names)
