    ...
    def callback(self, data):
        ...
        if self.sense:
            ...
            # Last time in sense phase
                ...
                print('tau_er_considered',tau_er_considered)
                if np.size(tau_er_considered) > 0:
                    self.mean_tau_er = np.sum(tau_er_considered) / np.size(tau_er_considered)
                    print('self.mean_tau_er_1',self.mean_tau_er)
                    self.mean_tau_er = np.sum((1/tau_er_considered*(np.std(tau_er_considered))**4))/np.sum(1/(tau_er_considered**2)*(np.std(tau_er_considered)**4))
                    print('self.mean_tau_er_2',self.mean_tau_er)
                else:
                    self.extreme_right = False # no TTT value in that ROI

                tau_el_considered = self.final_left_e[int(self.percentage * np.size(self.final_left_e)):np.size(self.final_left_e)]
                ...
                if np.size(tau_el_considered) > 0:
                    ...
                    print('self.mean_tau_el_1',self.mean_tau_el)
                    self.mean_tau_el = np.sum((1/tau_el_considered*(np.std(tau_el_considered))**4))/np.sum(1/(tau_er_considered**2)*(np.std(tau_el_considered))**4)
                    print('self.mean_tau_el_2',self.mean_tau_el)
                else:
                ...
                print('tau_r_considered',tau_r_considered)  
                if np.size(tau_r_considered) > 0:
                    self.mean_tau_r = np.sum(tau_r_considered) / np.size(tau_r_considered)
                    print('self.mean_tau_r_1',self.mean_tau_r)
                    self.mean_tau_r = np.sum((1/tau_r_considered*(np.std(tau_r_considered))**4))/np.sum(1/(tau_er_considered**2)*(np.std(tau_r_considered))**4)
                    print('self.mean_tau_r_2',self.mean_tau_r)
                else:
                    self.right = False

                tau_l_considered = self.final_left[int(self.percentage * np.size(self.final_left)):
                                                   np.size(self.final_left)]
                print('tau_l_considered',tau_l_considered)  
                if np.size(tau_l_considered) > 0:
                    self.mean_tau_l = np.sum(tau_l_considered) / np.size(tau_l_considered)
                    print('self.mean_tau_l_1',self.mean_tau_l)
                    self.mean_tau_l = np.sum((1/tau_l_considered*(np.std(tau_l_considered))**4))/np.sum(1/(tau_er_considered**2)*(np.std(tau_l_considered))**4)
                    print('self.mean_tau_l_2',self.mean_tau_l)
                else:
                    self.left = False
                ...
                if np.size(tau_center_considered) > 0:
                    ...
                    print('self.mean_tau_c_1',self.mean_tau_center)
                    self.mean_tau_center = np.sum((1/tau_center_considered*(np.std(tau_center_considered))**4))/np.sum(1/(tau_er_considered**2)*(np.std(tau_center_considered))**4)
                    print('self.mean_tau_c_2',self.mean_tau_center)
                else:
                    self.center = False
