...
class OFCalculator:
	...
	def callback(self, data):
		...
		if self.prev_image is None:
			...
			# creating ROI
			self.roi_el = curr_image[y_init_el:y_end_el, x_init_el:(x_end_el + int(data.width / 10))]
			self.roi_er = curr_image[y_init_er:y_end_er, (x_init_er - int(data.width / 10)):x_end_er]
			self.roi_l = curr_image[y_init_l:y_end_l, (x_init_l - int(data.width / 10)):x_end_l]
			self.roi_r = curr_image[y_init_r:y_end_r, x_init_r:(x_end_r + int(data.width /10))]
			self.roi_c = curr_image[y_init_c:y_end_c, x_init_c:x_end_c]
			# self.roi_el = curr_image[y_init_el:y_end_el, x_init_el:x_end_el]
			# self.roi_er = curr_image[y_init_er:y_end_er, x_init_er:x_end_er]
			# self.roi_l = curr_image[y_init_l:y_end_l, x_init_l:x_end_l]
			# self.roi_r = curr_image[y_init_r:y_end_r, x_init_r:x_end_r]
			# self.roi_c = curr_image[y_init_c:y_end_c, x_init_c:x_end_c]
		...
		msg.vx = flow[:, 0] / dt
		msg.vy = flow[:, 1] / dt
		msg.x_proj = good_kps_old[:, 0] - good_kps_new[:,0]
