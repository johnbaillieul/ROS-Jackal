...
plt.savefig(path_er + 'er_error_array_' + filter + str(count) +".png")
...
plt.savefig(path_el + 'el_error_array_' + filter + str(count) +".png")
...
plt.savefig(path_l + 'l_error_array_' + filter + str(count) +".png")
...
plt.savefig(path_r + 'r_error_array_' + filter + str(count) +".png")
...
plt.savefig(path_c + 'c_error_array_' + filter + str(count) +".png")

plt.show()

            # # seaborn histogram
            # sns.distplot(er_error_array,hist=True, kde=False, 
            #             bins=int(180/5), color = 'blue',
            #             hist_kws={'edgecolor':'black'})
            # # Add labels
            # plt.title('Histogram of error in el')
            # plt.xlabel('reading')
            # plt.ylabel('error')
            # plotting the points
            ...
            #     # Draw the plot
            #     ax.hist(roi, bins = int(180/5),
            #             color = 'blue', edgecolor = 'black')
                
            #     # Title and labels
            #     ax.set_title('Histogram of error in = %s' % roi, size = 30)
            #     ax.set_xlabel('reading', size = 22)
            #     ax.set_ylabel('error', size= 22)

            # plt.tight_layout()
            # plt.show()

            # # List of five roi to plot
            # Region_of_interest = ['extreme right', 'right', 'center',
            #         'left', 'extreme left']

            # # Iterate through the five roi
