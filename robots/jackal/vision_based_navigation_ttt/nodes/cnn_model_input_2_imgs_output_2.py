...
class train():
    ...
    def train_(self):
        ...
        output_1 = keras.layers.Dense(5,kernel_initializer='he_uniform')(drop_1)
        output_2 = keras.layers.Dense(5,kernel_initializer='he_uniform')(drop_1)
        ...
        with open('model_pkl_6.pkl', 'wb') as files: pickle.dump(model, files)


        # print(model.predict(X_test)[8])
        # y_1_test_loss, y_1_test_acc, y_2_test_loss, y_2_test_acc = model.evaluate({"input_1": X_test, "input_2": v_test}, [y_1_test, y_2_test])
        # # print('test loss y1: {}, test accuracy y1: {}, test loss y2: {}, test accuracy y2: {}'.format(y_1_test_loss, y_1_test_acc, y_2_test_loss, y_2_test_acc) )
...
class calc_tau():
    def __init__(self):
        ...
        self.model = pickle.load(open('model_pkl_6.pkl', 'rb'))
        # Lidar Subscriber
    ...
    def get_tau_values(self):
        if self.curr_image is not None:
            if self.prev_image is not None:
                ...
                if tau_pred[1][0][0] > 0.5:
                    tau_pred_el = tau_pred[0][0][0]
                else: 
                    tau_pred_el = -1
            
                if tau_pred[1][0][4] > 0.5:
                    tau_pred_er = tau_pred[0][0][4]
                else: 
                    tau_pred_er = -1
                    
                if tau_pred[1][0][1] > 0.5:
                    tau_pred_l = tau_pred[0][0][1]
                else: 
                    tau_pred_l = -1

                if tau_pred[1][0][3] > 0.5:
                    tau_pred_r = tau_pred[0][0][3]
                else: 
                    tau_pred_r = -1

                if tau_pred[1][0][2] > 0.5:
