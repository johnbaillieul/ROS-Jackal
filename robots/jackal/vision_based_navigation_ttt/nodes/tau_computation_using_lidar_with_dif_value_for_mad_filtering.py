...
# Filtering procedure for the TTT values using Mean absolute deviation
def tau_filtering_mad(vector, filter):  
...
# Filtering procedure for the TTT values by discarding a percentage of the highest and lowest values
def tau_filtering_discarding_percantage(vector):
...
class TauComputationClass:
    ...
    def __init__(self):
        ...
        self.of_sub = rospy.Subscriber("optical_flow", OpticalFlow, self.callback_OFS)
        ...
        self.er_error_array_1 = []
        self.r_error_array_1 = []
        self.c_error_array_1 = []
        self.l_error_array_1 = []
        self.el_error_array_1 = []

        self.er_error_array_2 = []
        self.r_error_array_2 = []
        self.c_error_array_2 = []
        self.l_error_array_2 = []
        self.el_error_array_2 = []

        self.er_error_array_3 = []
        self.r_error_array_3 = []
        self.c_error_array_3 = []
        self.l_error_array_3 = []
        self.el_error_array_3 = []

        self.er_error_array_4 = []
        self.r_error_array_4 = []
        self.c_error_array_4 = []
        self.l_error_array_4 = []
        self.el_error_array_4 = []
    ...
    def callback_OF(self, data):
        final_tau_left_e_1, final_tau_left_1, final_tau_centre_1, final_tau_right_1, final_tau_right_e_1 = self.callback_of_range(data,1)
        final_tau_left_e_2, final_tau_left_2, final_tau_centre_2, final_tau_right_2, final_tau_right_e_2 = self.callback_of_range(data,20)
        final_tau_left_e_3, final_tau_left_3, final_tau_centre_3, final_tau_right_3, final_tau_right_e_3 = self.callback_of_range(data,50)
        final_tau_left_e_4, final_tau_left_4, final_tau_centre_4, final_tau_right_4, final_tau_right_e_4 = self.callback_of_range(data,100)
        ...
        if self.ranges is not None:
            ...
            relative_error_el_1, relative_error_l_1, relative_error_c_1, relative_error_r_1, relative_error_er_1 = self.error(tau_el, final_tau_left_e_1, tau_l, final_tau_left_1,tau_c , final_tau_centre_1, tau_r, final_tau_right_1,tau_er, final_tau_right_e_1)
            
            if abs(tau_el) == np.inf :
                pass
            elif final_tau_left_e_1 == -1:
                self.el_error_array_1.append(tau_el)
            else:
                self.el_error_array_1.append(relative_error_el_1)

            if abs(tau_l) == np.inf :
                pass
            elif final_tau_left_1 == -1:
                self.l_error_array_1.append(tau_l)
            else:
                self.l_error_array_1.append(relative_error_l_1)

            if abs(tau_c) == np.inf :
                pass
            elif final_tau_centre_1 == -1:
                self.c_error_array_1.append(tau_c)
            else:
                self.c_error_array_1.append(relative_error_c_1)

            if abs(tau_r) == np.inf :
                pass
            elif final_tau_right_1 == -1:
                self.r_error_array_1.append(tau_r)
            else:
                self.r_error_array_1.append(relative_error_r_1)

            if abs(tau_er) == np.inf :
                pass
            elif final_tau_right_e_1 == -1:
                self.er_error_array_1.append(tau_er)
            else:
                self.er_error_array_1.append(relative_error_er_1)

            relative_error_el_2, relative_error_l_2, relative_error_c_2, relative_error_r_2, relative_error_er_2 = self.error(tau_el, final_tau_left_e_2, tau_l, final_tau_left_2,tau_c , final_tau_centre_2, tau_r, final_tau_right_2,tau_er, final_tau_right_e_2)
            
            if abs(tau_el) == np.inf :
                pass
            elif final_tau_left_e_2 == -1:
                self.el_error_array_2.append(tau_el)
            else:
                self.el_error_array_2.append(relative_error_el_2)

            if abs(tau_l) == np.inf :
                pass
            elif final_tau_left_2 == -1:
                self.l_error_array_2.append(tau_l)
            else:
                self.l_error_array_2.append(relative_error_l_2)

            if abs(tau_c) == np.inf :
                pass
            elif final_tau_centre_2== -1:
                self.c_error_array_2.append(tau_c)
            else:
                self.c_error_array_2.append(relative_error_c_2)

            if abs(tau_r) == np.inf :
                pass
            elif final_tau_right_2 == -1:
                self.r_error_array_2.append(tau_r)
            else:
                self.r_error_array_2.append(relative_error_r_2)

            if abs(tau_er) == np.inf :
                pass
            elif final_tau_right_e_2 == -1:
                self.er_error_array_2.append(tau_er)
            else:
                self.er_error_array_2.append(relative_error_er_2)

            relative_error_el_3, relative_error_l_3, relative_error_c_3, relative_error_r_3, relative_error_er_3 = self.error(tau_el, final_tau_left_e_3, tau_l, final_tau_left_3,tau_c , final_tau_centre_3, tau_r, final_tau_right_3,tau_er, final_tau_right_e_3)
            
            if abs(tau_el) == np.inf :
                pass
            elif final_tau_left_e_3 == -1:
                self.el_error_array_3.append(tau_el)
            else:
                self.el_error_array_3.append(relative_error_el_3)

            if abs(tau_l) == np.inf :
                pass
            elif final_tau_left_3 == -1:
                self.l_error_array_3.append(tau_l)
            else:
                self.l_error_array_3.append(relative_error_l_3)

            if abs(tau_c) == np.inf :
                pass
            elif final_tau_centre_3== -1:
                self.c_error_array_3.append(tau_c)
            else:
                self.c_error_array_3.append(relative_error_c_3)

            if abs(tau_r) == np.inf :
                pass
            elif final_tau_right_3 == -1:
                self.r_error_array_3.append(tau_r)
            else:
                self.r_error_array_3.append(relative_error_r_3)

            if abs(tau_er) == np.inf :
                pass
            elif final_tau_right_e_3 == -1:
                self.er_error_array_3.append(tau_er)
            else:
                self.er_error_array_3.append(relative_error_er_3)

            relative_error_el_4, relative_error_l_4, relative_error_c_4, relative_error_r_4, relative_error_er_4 = self.error(tau_el, final_tau_left_e_4, tau_l, final_tau_left_4,tau_c , final_tau_centre_4, tau_r, final_tau_right_4,tau_er, final_tau_right_e_4)
            if abs(tau_el) == np.inf :
                pass
            elif final_tau_left_e_4 == -1:
                self.el_error_array_4.append(tau_el)
            else:
                self.el_error_array_4.append(relative_error_el_4)

            if abs(tau_l) == np.inf :
                pass
            elif final_tau_left_4 == -1:
                self.l_error_array_4.append(tau_l)
            else:
                self.l_error_array_4.append(relative_error_l_4)

            if abs(tau_c) == np.inf :
                pass
            elif final_tau_centre_4== -1:
                self.c_error_array_4.append(tau_c)
            else:
                self.c_error_array_4.append(relative_error_c_4)

            if abs(tau_r)== np.inf :
                pass
            elif final_tau_right_4 == -1:
                self.r_error_array_4.append(tau_r)
            else:
                self.r_error_array_4.append(relative_error_r_4)

            if abs(tau_er) == np.inf :
                pass
            elif final_tau_right_e_4 == -1:
                self.er_error_array_4.append(tau_er)
            else:
                self.er_error_array_4.append(relative_error_er_4)

    def callback_of_range(self, data, range_):
        ...
        tau_right_e = tau_filtering_mad(tau_right_e,range_)
        tau_left_e = tau_filtering_mad(tau_left_e,range_)
        # Right and left
        tau_right = tau_filtering_mad(tau_right,range_)
        tau_left = tau_filtering_mad(tau_left,range_)
        # Centre
        tau_centre = tau_filtering_mad(tau_centre,range_)
        # Extreme right and left
        ...
        return final_tau_left_e, final_tau_left, final_tau_centre ,final_tau_right,final_tau_right_e

    
            
    def error(self,tau_el, final_tau_left_e, tau_l, final_tau_left,tau_c , final_tau_centre, tau_r, final_tau_right,tau_er, final_tau_right_e):
        relative_error_er = (tau_er - final_tau_right_e )
        relative_error_r = (tau_r - final_tau_right)
        relative_error_c = (tau_c - final_tau_centre)
        relative_error_l = (tau_l - final_tau_left)
        relative_error_el = (tau_el - final_tau_left_e)
        ...
        return relative_error_el, relative_error_l, relative_error_c, relative_error_r, relative_error_er
    ...
    def save_data(self, er_error_array, r_error_array, c_error_array, l_error_array, el_error_array, count, filter):
            path_folder = os.environ["HOME"] + "/catkin_ws/src/vision_based_navigation_ttt/mad_ranges_1/"
            folder_name = filter + str(count) + '/'
            os.mkdir(path_folder + folder_name) 

            er_error = pd.DataFrame(er_error_array)
            # filepath_er = os.environ["HOME"]+"/catkin_ws/src/vision_based_navigation_ttt/csv/er_error_array.xlsx"
            er_error.to_excel(path_folder + folder_name + 'er_error_array_' + filter + '.xlsx')

            el_error = pd.DataFrame(el_error_array)
            # filepath_el = os.environ["HOME"]+"/catkin_ws/src/vision_based_navigation_ttt/csv/el_error_array.xlsx"
            el_error.to_excel(path_folder + folder_name + 'el_error_array_' + filter + '.xlsx')

            r_error = pd.DataFrame(r_error_array)
            # filepath_r = os.environ["HOME"]+"/catkin_ws/src/vision_based_navigation_ttt/csv/r_error_array.xlsx"
            r_error.to_excel(path_folder + folder_name + 'r_error_array_' + filter + '.xlsx')

            l_error = pd.DataFrame(l_error_array)
            # filepath_l = os.environ["HOME"]+"/catkin_ws/src/vision_based_navigation_ttt/csv/l_error_array.xlsx"
            l_error.to_excel(path_folder + folder_name + 'l_error_array_' + filter + '.xlsx')

            c_error = pd.DataFrame(c_error_array)
            # filepath_c = os.environ["HOME"]+"/catkin_ws/src/vision_based_navigation_ttt/csv/c_error_array.xlsx"
            c_error.to_excel(path_folder + folder_name + 'c_error_array_' + filter + '.xlsx')

    def on_rospy_shutdown(self):
            rospy.logwarn("Stopping")
            rospy.Rate(1).sleep()
            count = 4
            self.save_data(self.er_error_array_1, self.r_error_array_1, self.c_error_array_1, self.l_error_array_1, self.el_error_array_1, count, 'mad_range_1_')
            self.save_data(self.er_error_array_2, self.r_error_array_2, self.c_error_array_2, self.l_error_array_2, self.el_error_array_2, count, 'mad_range_20_')
            self.save_data(self.er_error_array_3, self.r_error_array_3, self.c_error_array_3, self.l_error_array_3, self.el_error_array_3, count, 'mad_range_50_')
            self.save_data(self.er_error_array_4, self.r_error_array_4, self.c_error_array_4, self.l_error_array_4, self.el_error_array_4, count, 'mad_range_100_')
