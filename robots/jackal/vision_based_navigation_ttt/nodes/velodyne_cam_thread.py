...
class TauComp:
    def __init__(self):
        ...
        self.points_arr_el_median = None
        self.points_arr_l_median = None
        self.points_arr_c_median= None
        self.points_arr_r_median = None
        self.points_arr_er_median = None
    ...
    def get_median(self, points_arr, points_arr_median):
            minimum = 5
            # print('points_arr',points_arr)
            print('size of points_arr',np.size(points_arr[:,0]))
            if np.size(points_arr[:,0]) > minimum:
                print('points_arr in if',np.size(points_arr[:,0]))
                points_arr_median = np.median(points_arr[:,0])
            else:
                points_arr_median = -1

            # print("med_el", points_arr_median)
    ...
    def cloud_callback(self, cloud):
        if self.curr_image is not None:
            ...
            if cloud is not None:
                ...
                if np.size(points_arr_[:,0]) > 0:
                    print('size', np.size(points_arr_[:,0]))
                    # print(points_arr_)
                    leng = len(points_arr[:,0])
                    ...
                    # create threads
                    points_arr_el_median = Thread(target= self.get_median, args=(points_arr_el, self.points_arr_el_median ))
                    points_arr_er_median = Thread(target= self.get_median, args=(points_arr_er, self.points_arr_el_median ))
                    points_arr_r_median = Thread(target= self.get_median, args=(points_arr_r,self.points_arr_el_median ))
                    points_arr_l_median = Thread(target= self.get_median, args=(points_arr_l, self.points_arr_el_median))
                    points_arr_c_median = Thread(target= self.get_median, args=(points_arr_c, self.points_arr_el_median ))

                    # start the threads
                    points_arr_el_median.start()
                    points_arr_er_median.start()
                    points_arr_r_median.start()
                    points_arr_l_median.start()
                    points_arr_c_median.start()

                    # wait for the threads to complete
                    points_arr_el_median.join()
                    points_arr_er_median.join()
                    points_arr_r_median.join()
                    points_arr_l_median.join()
                    points_arr_c_median.join()

                    print('points_arr_el_median',self.points_arr_el_median)
                    print('points_arr_er_median',self.points_arr_er_median)
                    print('points_arr_r_median',self.points_arr_r_median)
                    print('points_arr_l_median',self.points_arr_l_median)
                    print('points_arr_c_median',self.points_arr_c_median)
                    ...
                    msg.tau_el = self.points_arr_el_median
                    msg.tau_er = self.points_arr_er_median
                    msg.tau_l = self.points_arr_l_median
                    msg.tau_r = self.points_arr_r_median
                    msg.tau_c = self.points_arr_c_median
                    self.tau_values.publish(msg)

                    # Draw the ROIs with their TTT values
                    draw_image_segmentation(curr_image, self.points_arr_el_median , self.points_arr_er_median, self.points_arr_l_median, self.points_arr_r_median, self.points_arr_c_median)
