...
path_image = path + "test_data_img/training_images_304_v_1/"
# dataset_name = 'test_data_img/training_images_306_v_1'
first_image_no = 46741
last_image_no = 46943

range_frames = (first_image_no, last_image_no)

model = tf.keras.models.load_model(path + "trained_model_parameters/renest101v2_model_64_updated.h5", compile=False, custom_objects=ak.CUSTOM_OBJECTS)
...
tau_lleft_pred = [tmp[0] for tmp in res]
...
np.save(path +'/dnn_results/DNN_tau_le_str.npy', tau_lleft_pred)
np.save(path +'/dnn_results/DNN_tau_l_str.npy', tau_left_pred)
np.save(path +'/dnn_results/DNN_tau_c_str.npy', tau_center_pred)
np.save(path +'/dnn_results/DNN_tau_r_str.npy', tau_right_pred)
np.save(path +'/dnn_results/DNN_tau_re_str.npy', tau_rright_pred)